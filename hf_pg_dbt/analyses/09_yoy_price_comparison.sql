...
SELECT
    ...
    surface_area_m2_normalised,
    order_cost_eur,
    (order_cost_eur / meals_count) AS cost_per_meal_eur
FROM with_area_cost
WHERE market = 'DE' AND pkg_id = 'P-S'
...
SELECT
    ...
    (
        ...
        / NULLIF(MIN(CASE WHEN yr = 2025 THEN cost_per_m2_eur END), 0) * 100) AS yoy_rate_change_pct
FROM with_area_cost
GROUP  BY market
