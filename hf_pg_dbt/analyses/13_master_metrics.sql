-- ============================================================
-- 13 · MASTER METRICS VIEW — ALL KPIs IN ONE QUERY
-- HelloFresh Packaging Analysis
-- Single query producing every headline metric referenced
...
enriched AS (
    ...
    LEFT  JOIN dim_packaging_standards ds ON d.meals_count  = ds.meals_count
...
SELECT
    ...
    -- ── Cost Inefficiency (over-boxing waste) ─────────────────
    ROUND(SUM(actual_cost_eur) - SUM(ideal_cost_eur), 4)
                                                    AS total_cost_inefficiency_eur,
    ...
    ROUND(SUM(ideal_cost_eur)
          / NULLIF(SUM(actual_cost_eur), 0) * 100, 1)
                                                    AS overall_cost_efficiency_pct,

    -- ── Cost Per Meal ─────────────────────────────────────────
    ROUND(SUM(actual_cost_eur) / NULLIF(SUM(meals_count), 0), 4)
                                                    AS overall_actual_cpm_eur,
    ROUND(SUM(ideal_cost_eur)  / NULLIF(SUM(meals_count), 0), 4)
                                                    AS overall_ideal_cpm_eur,
    ...
    -- ── Data Quality ──────────────────────────────────────────
    (SELECT COUNT(*) FROM fact_box_usage WHERE pkg_id IS NULL)
                                                    AS null_pkg_id_orders,
    (SELECT COUNT(*) - COUNT(DISTINCT order_id)
     FROM   fact_box_usage WHERE pkg_id IS NOT NULL)
